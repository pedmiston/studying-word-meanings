...
```{r dog, fig.cap = ""}
dog <- "img/dog.png" %>% readPNG %>% rasterGrob(x = 0.6, width = 0.5)
grid.newpage()
grid.draw(dog)
...
```{r label_advantage}
grid.newpage()
grid.draw("img/lupyan_thompsonschill_2012_jepg/fig_1a.png" %>% readPNG %>% rasterGrob(height = 0.8))
grid.text("Lupyan & Thompson-Schill 2012 JEP:G", y = 0.9, gp = gp)
```
...
pictures <- c("guitar_acoustic", "guitar_electric") %>%
  lapply(function (x) paste0(x, ".jpg") %>% file.path("img", .) %>% readJPEG)
grid.newpage()
grid.draw(rasterGrob(pictures[[1]], x = 0.12, width = 0.5))
grid.draw(rasterGrob(pictures[[2]], x = 0.88, width = 0.5))
